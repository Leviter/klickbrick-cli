...
class Test(TestCase):
    ...
    def test_get_named_greeting(self):
        self.assertEqual(hello.get_greeting(['--name', 'Charlie']), "hello Charlie")

    def test_get_empty_name_greeting(self):
        self.assertEqual(hello.get_greeting(['--name', '']), "hello ")

    def test_get_args_name(self):
        parsed_arguments = hello.get_args(['--name', 'Michael'])
